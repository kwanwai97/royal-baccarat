...
const db = new Database(path.join(DATA_DIR, "users.sqlite"));
...
const updateUser = db.prepare(
  "UPDATE users SET credits=?, revival=?, ads_watched=? WHERE id=?"
);
const insertTransaction = db.prepare(
  "INSERT INTO transactions(user_id,type,amount,note) VALUES(?,?,?,?)"
);
const upsertSession = db.prepare(
  "INSERT INTO sessions(token,user_id,expires) VALUES(?,?,?) ON CONFLICT(token) DO UPDATE SET user_id=excluded.user_id, expires=excluded.expires"
);
const selectSession = db.prepare(
  "SELECT s.user_id, s.expires, u.username FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token=?"
);
...
app.post("/api/register", (req, res) => {
  ...
  if (username.length < 3 || username.length > 20) {
    return res.status(400).json({ error: "帳號需 3-20 個字" });
  }
  if (password.length < 4) {
    return res.status(400).json({ error: "密碼至少 4 個字" });
  }
  ...
  upsertSession.run(token, user.id, expires);
  res.json({
    token,
    user: {
      id: user.id,
...
app.post("/api/revive", requireAuth, (req, res) => {
  const user = selectUser.get(req.user.id);
  if (user.revival >= 3) {
    return res.status(400).json({ error: "本局已用完復活" });
  }
  const newRevival = user.revival + 1;
  updateUser.run(user.credits, newRevival, user.ads_watched, user.id);
  insertTransaction.run(user.id, "revive", 0, `第${newRevival}次復活`);
...
app.post("/api/watch-ad", requireAuth, (req, res) => {
  ...
  updateUser.run(newCredits, user.revival, newAds, user.id);
...
app.post("/api/purchase", requireAuth, (req, res) => {
  ...
  if (!product) {
    return res.status(400).json({ error: "不存在的商品" });
  }
  let newCredits = user.credits + (product.credits || 0);
  let newRevival = user.revival + (product.revival || 0);
  updateUser.run(newCredits, newRevival, user.ads_watched, user.id);
  insertTransaction.run(
    user.id,
    "purchase",
    product.credits || 0,
...
app.get("/api/leaderboard", (req, res) => {
  const rows = db
    .prepare(
      "SELECT username, credits, revival FROM users ORDER BY credits DESC LIMIT 20"
    )
    .all();
  res.json(rows);
});
